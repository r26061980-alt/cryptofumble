...
exports.handler = async (event) => {
  ...
  if (type === 'top10') {
    ...
    //
    // CoinGecko иногда включает в market_cap_desc токены, которые технически
    // не являются "криптовалютой" в привычном смысле — например, FIGR_HELOC
    // (токенизированный продукт HELOC от компании Figure, привязанный к цене
    // ~$1). Первая попытка фильтровать это по "объём/капитализация < 0.1%"
    // не сработала: 01.09.2026 проверка на живых данных показала, что у
    // FIGR_HELOC соотношение объёма к капитализации — 3%, что ВЫШЕ порога и
    // сравнимо с обычными монетами (XRP — 2.1%, TRX — 1.4%). То есть по
    // ликвидности он не аномален, и объёмный фильтр его не ловит.
    //
    // Настоящая причина расхождения с CoinMarketCap — не ликвидность, а то,
    // что CMC вообще не считает такие токенизированные финансовые продукты
    // "криптовалютой" и держит их вне топ-листа (у FIGR_HELOC там ранг
    // #3930, несмотря на капитализацию, которая по чистой сортировке
    // CoinGecko ставит его на 9-е место). Это не вычисляется по формуле —
    // это ручная классификация. Поэтому вместо объёмного порога — явный
    // список исключений по id CoinGecko. Топорно, но честно: если завтра
    // появится другой такой токен, ежедневная автопроверка (см. growth
    // ledger, Фаза 17) заметит его и сообщит, а сюда его добавит человек.
    const EXCLUDED_IDS = new Set([
      'figure-heloc', // FIGR_HELOC — токенизированный HELOC, не криптовалюта в обычном смысле
    ]);
    // Как дополнительная защита (на случай будущих настоящих аномалий с
    // объёмом) — тот же порог 0.1%, что и раньше, просто он не решает
    // именно этот случай.
    try {
      const key = 'top10';
